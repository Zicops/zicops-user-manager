...
// GetAllVendors is the resolver for the getAllVendors field.
func (r *queryResolver) GetAllVendors(ctx context.Context, vendorIds []*string) ([]*model.Vendor, error) {
	res, err := handlers.GetAllVendors(ctx, vendorIds)
	if err != nil {
		log.Printf("error getting vendors: %v", err)
		return nil, err
	}
	return res, nil
}
...
// GetCourseAnalyticsDataByID is the resolver for the getCourseAnalyticsDataById field.
func (r *queryResolver) GetCourseAnalyticsDataByID(ctx context.Context, courseID *string, status *string) (*model.CourseAnalyticsFacts, error) {
	res, err := handlers.GetCourseAnalyticsDataById(ctx, courseID, status)
	if err != nil {
		log.Printf("error getting course anaylytics by courseId: %v", err)
		return nil, err
	}
	return res, nil
}

// GetLearnerDetails is the resolver for the getLearnerDetails field.
func (r *queryResolver) GetLearnerDetails(ctx context.Context, courseID *string, pageCursor *string, direction *string, pageSize *int) (*model.PaginatedUserCourseAnalytics, error) {
	res, err := handlers.GetLearnerDetails(ctx, courseID, pageCursor, direction, pageSize)
	if err != nil {
		log.Printf("error getting course learners details: %v", err)
		return nil, err
	}
	return res, nil
}
