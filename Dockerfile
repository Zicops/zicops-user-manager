...
RUN go mod download
COPY . .
RUN CGO_ENABLED=1 GOOS=linux go build -a -installsuffix cgo -o zicops-user-manager .
...
COPY --from=builder /go/src/app/zicops-user-manager /usr/bin/
EXPOSE 8094

ENTRYPOINT ["/usr/bin/zicops-user-manager"]
